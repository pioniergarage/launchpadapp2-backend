...
ON DUPLICATE KEY UPDATE 
    `here_since`=IF(`last_seen` + INTERVAL " . mysqli_escape_string($conn, strval($timeout)) . " MINUTE < CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, `here_since`), 
...
WHERE `here_since` < CURRENT_TIMESTAMP - INTERVAL 1 DAY 
  AND `last_seen` > CURRENT_TIMESTAMP - INTERVAL " . mysqli_escape_string($conn, strval($timeout)) . " MINUTE 
  AND `blacklisted`=0");
        if ($result == false) set_status_code(400);
...
//GET USERS THAT HAVE BEEN AT THE LAUNCHPAD IN THE LAST TIME INTERVAL
...
WHERE `blacklisted`=0 
  AND `user_id` IS NULL 
  AND `last_seen` > CURRENT_TIMESTAMP - INTERVAL " . mysqli_escape_string($conn, strval($timeout)) . " MINUTE")->fetch_assoc()["amount"]);

    //get all users whose mac address has been tracked in the last interval
...
WHERE 
	here_since =  ( SELECT MIN(here_since) FROM presence_tracker_macs 
                   ...
                   AND presence_tracker_macs.last_seen > CURRENT_TIMESTAMP - INTERVAL " . mysqli_escape_string($conn, strval($timeout)) . " MINUTE )
AND
    last_seen > CURRENT_TIMESTAMP - INTERVAL " . mysqli_escape_string($conn, strval($timeout)) . " MINUTE");
    $ouput['users'] = array();
...
if (isset($_GET['newUser'])) {
    ...
    if (!isset($_POST['first_name']) or empty($_POST['first_name'])
        or !isset($_POST['last_name']) or empty($_POST['last_name'])
        or !isset($_POST['orga_name']) or empty($_POST['orga_name'])
        or !isset($_POST['profile_url']) or empty($_POST['profile_url'])
        or !isset($_POST['mac1']) or empty($_POST['mac1'])) {
        set_status_code(400);
    }
    $user_id = -1;
...
if (isset($_GET['activeMacs']) && $debug) {
    $data_macs = $conn->query("SELECT * FROM `presence_tracker_macs` WHERE `last_seen` > CURRENT_TIMESTAMP - INTERVAL " . mysqli_escape_string($conn, $timeout) . " MINUTE ORDER BY `here_since`");
    $macs = array();
    while ($row = $data_macs->fetch_assoc()) {
        array_push($macs, $row);
    }
